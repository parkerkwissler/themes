...
function educator_page_title( $page ) {
	switch ( $page ) {
		...
		case 'search':
			...
			<div id="page-title">
				<h1>
					<?php printf( __( 'Search Results for: %s', 'ib-educator' ), get_search_query() ); ?>
				</h1>
...
function educator_paging_nav( $num_pages = null ) {
	...
	foreach ( $paginate_links as $link ) echo $link;
	echo '</div>';
	echo '</div>';
}
endif;
...
function educator_post_meta( $include = null ) {
	...
	if ( in_array( 'date', $include ) ) {
		$meta .= sprintf(
			'<span class="post-date"><time class="entry-date" datetime="%1$s">%2$s</time></span>',
